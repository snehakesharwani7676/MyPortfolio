...
const SkinNailsPortfolio = () => {
  ...
  const fetchPortfolio = async () => {
    try {
      ...
      // Filter only skin/nails categories
      const skinNailsItems = response.data.filter(item => 
        categories.includes(item.category)
      );
      setPortfolioItems(skinNailsItems);
    } catch (error) {
    ...
    <div className="portfolio-page">
      <div className="portfolio-header">
        <div className="container">
          <h1>Skin & Nails Portfolio</h1>
          <p>Professional skin care and nail art services</p>
        </div>
      </div>
...
export default SkinNailsPortfolio;
